...
从文件名中提取最后一个8位数字作为候选日期，校验有效性后使用。
...
如果最后一个候选日期无效，则继续向前查找前一个有效日期。

### 11.3 处理流程

```
文件名: scan_12026052_test120260520.xlsx

Step 1: 提取所有8位数字候选
  - 候选1: 12026052 (从 scan_12026052)
  - 候选2: 20260520 (从 test120260520)

Step 2: 从后向前校验有效性
  - 候选2: 20260520 → 月=06, 日=20 → ✅ 有效
  - 结果: pt_dt = '2026-05-20'
```

### 11.4 兜底处理

如果所有候选日期都无效，则使用当前系统日期作为默认值。
