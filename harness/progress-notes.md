...
### 2026-04-29 - FTP配置简化重构

**会话目标**: 废弃多FTP设计方案，统一采用内置FTP，扫描路径迁移至报表配置

**需求背景**:
1. 移除FTP配置中的扫描路径设置项，FTP配置仅保留连接相关参数
2. 废弃多FTP设计方案，统一采用当前项目的自有FTP作为默认连接
3. 实现FTP用户名和密码通过配置文件进行管理的机制
4. 删除FTP管理页面及其相关功能模块
5. 扫描路径配置迁移至报表配置模块

**设计方案**:
- 方案选择：激进式重构（完全移除外部FTP配置体系）
- 内置FTP作为独立模块，通过配置文件管理
- 扫描路径通过 `report_config.scan_path` 字段配置

**设计文档**: [docs/superpowers/specs/2026-04-29-ftp-simplification-design.md](file:///home/nova/projects/report-front/docs/superpowers/specs/2026-04-29-ftp-simplification-design.md)

**实施计划**: [docs/superpowers/plans/2026-04-29-ftp-simplification-plan.md](file:///home/nova/projects/report-front/docs/superpowers/plans/2026-04-29-ftp-simplification-plan.md)

**执行任务**:
| 任务ID | 任务名称 | 状态 | 备注 |
|--------|----------|------|------|
| Task 1 | 删除FtpConfig后端代码 | ✅ 完成 | 6个文件删除 |
| Task 2 | 修改FtpScanJob | ✅ 完成 | 移除外部FTP扫描 |
| Task 3 | 修改ReportConfig实体 | ✅ 完成 | 新增scanPath字段 |
| Task 4 | 修改ReportConfigController | ✅ 完成 | 使用内置FTP |
| Task 5 | 修改TaskController | ✅ 完成 | 使用内置FTP |
| Task 6 | 修改数据库Schema | ✅ 完成 | schema.sql, schema-gaussdb.sql |
| Task 7 | 删除前端FTP配置代码 | ✅ 完成 | FtpConfig.vue, ftpConfig.js |
| Task 8 | 修改ReportConfig.vue | ✅ 完成 | 新增扫描路径配置 |
| Task 9 | 数据迁移脚本 | ✅ 完成 | V1.0__ftp_simplification.sql |
| Task 10 | 同步harness上下文 | ✅ 完成 | tasks.json, progress-notes.md |

**代码变更清单**:

| 变更类型 | 文件 | 说明 |
|----------|------|------|
| 删除 | FtpConfigController.java | FTP配置REST API |
| 删除 | FtpConfigService.java | FTP配置服务接口 |
| 删除 | FtpConfigServiceImpl.java | FTP配置服务实现 |
| 删除 | FtpConfigMapper.java | FTP配置Mapper |
| 删除 | FtpConfigMapper.xml | MyBatis XML |
| 删除 | FtpConfig.java | FTP配置实体 |
| 删除 | FtpConfig.vue | FTP配置页面 |
| 删除 | ftpConfig.js | FTP API调用 |
| 删除 | /ftp 路由 | 移除FTP页面路由 |
| 修改 | FtpScanJob.java | 移除外部FTP扫描 |
| 修改 | ReportConfig.java | 新增scanPath字段 |
| 修改 | ReportConfigDTO.java | 新增scanPath字段 |
| 修改 | ReportConfigController.java | 使用内置FTP |
| 修改 | TaskController.java | 使用内置FTP |
| 修改 | ReportConfig.vue | 新增扫描路径配置 |
| 修改 | schema.sql | 移除ftp_config表，report_config新增scan_path |
| 修改 | schema-gaussdb.sql | 同上 |
| 新增 | migration/V1.0__ftp_simplification.sql | 数据迁移脚本 |
...
| 影响模块 | 说明 |
|----------|------|
| FTP管理 | 页面删除，外部FTP配置废弃 |
| 报表配置 | 新增扫描路径配置项 |
| 任务触发 | 扫描逻辑改为读取内置FTP |
| 定时任务 | FtpScanJob仅扫描内置FTP |
| 数据迁移 | 需执行迁移脚本 |

**Harness上下文同步检查**:
- ✅ tasks.json 任务状态已更新 (H-FTP-REFACTOR)
- ✅ progress-notes.md 会话记录已追加
- ✅ 设计文档已创建
- ✅ 实施计划已创建
- ✅ Git 已提交
