...
系统表统一使用 `sys_` 前缀（不纳入数据资产管理）：
```
sys_ftp_config, sys_trigger_config, sys_task_execution, sys_operation_log
```
...
数据中心模块扫描业务表时，只扫描以下前缀：`ods_`, `dwd_`, `dws_`, `ads_`, `dim_`, `mid_`, `tmp_`
...
| 2026-04-08 | V1.3 | [新增] 数据中心模块，添加数据表命名规范（ods/dwd/dws/ads/dim/mid/tmp） | AI Assistant |
