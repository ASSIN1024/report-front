...
class E2ETestRunner:
    ...
    def test_report_config(self):
        ...
        try:
            ...
            # 选择FTP配置
            self.page.click('.el-form:has(input[placeholder="如: SALES_REPORT"]) .el-select')
            self.page.wait_for_timeout(500)
            ...
            # 使用 JavaScript 直接操作 DOM 来添加列映射
            # 模拟点击"+ 添加映射"按钮
            add_btn = self.page.locator('button:has-text("+ 添加映射")')
            ...
            # 使用 evaluate 直接设置 Vue 组件的数据
            self.page.evaluate("""
                () => {
                    // 找到 el-table 中的 input 并设置值
                    const tables = document.querySelectorAll('.el-table');
                    if (tables.length > 1) {
                        const rows = tables[1].querySelectorAll('tbody tr');
                        const mappings = [
                            { col: 'A', field: 'col_a', type: 'STRING' },
                            { col: 'B', field: 'col_b', type: 'INTEGER' },
                            { col: 'C', field: 'col_c', type: 'DECIMAL' }
                        ];
                        rows.forEach((row, idx) => {
                            if (idx < mappings.length) {
                                const inputs = row.querySelectorAll('input');
                                if (inputs[0]) inputs[0].value = mappings[idx].col;
                                if (inputs[1]) inputs[1].value = mappings[idx].field;
                            }
                        });
                    }
                }
            ...
            # 验证
            self.page.wait_for_url("**/report", timeout=10000)
