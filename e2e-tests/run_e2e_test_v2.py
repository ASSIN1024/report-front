...
class E2ETestRunner:
    ...
    def test_report_config(self):
        ...
        try:
            ...
            # 填写第一个映射
            mapping_table = self.page.locator('.el-table').nth(1)
            mapping_table.locator('input[placeholder="如: A"]').first.fill('A')
            mapping_table.locator('input[placeholder="如: order_id"]').first.fill('col_a')
            mapping_table.locator('.el-select').first.click()
            self.page.wait_for_timeout(300)
    ...
    def test_scan_trigger(self):
        ...
        try:
            ...
            # 点击立即扫描
            scan_button = self.page.locator('button:has-text("立即扫描")').first
