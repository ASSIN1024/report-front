...
class E2ETestRunner:
    ...
    def test_report_config(self):
        ...
        try:
            ...
            # 填写输出表名
            self.page.fill('input[placeholder="如: t_sales_data"]', self.output_table)
            self.log(f"✅ 已填写输出表名: {self.output_table}")

            self.save_screenshot("11_report_form_filled")

            # 添加列映射 - 使用 press_sequence 模拟真实键盘输入
            for i in range(3):
            ...
            # 第一行 - 使用 evaluate 触发 Vue 双向绑定
            val_a1 = 'A'
            val_f1 = 'col_a'
            mapping_table.locator('input[placeholder="如: A"]').nth(0).evaluate(f"""
                (input) => {{
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                    nativeInputValueSetter.call(input, '{val_a1}');
                    input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                    input.dispatchEvent(new Event('change', {{ bubbles: true }}));
                }}
            """)
            mapping_table.locator('input[placeholder="如: order_id"]').nth(0).evaluate(f"""
                (input) => {{
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                    nativeInputValueSetter.call(input, '{val_f1}');
                    input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                    input.dispatchEvent(new Event('change', {{ bubbles: true }}));
                }}
            """)
            ...
            val_a2 = 'B'
            val_f2 = 'col_b'
            mapping_table.locator('input[placeholder="如: A"]').nth(1).evaluate(f"""
                (input) => {{
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                    nativeInputValueSetter.call(input, '{val_a2}');
                    input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                    input.dispatchEvent(new Event('change', {{ bubbles: true }}));
                }}
            """)
            mapping_table.locator('input[placeholder="如: order_id"]').nth(1).evaluate(f"""
                (input) => {{
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                    nativeInputValueSetter.call(input, '{val_f2}');
                    input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                    input.dispatchEvent(new Event('change', {{ bubbles: true }}));
                }}
            """)
            ...
            val_a3 = 'C'
            val_f3 = 'col_c'
            mapping_table.locator('input[placeholder="如: A"]').nth(2).evaluate(f"""
                (input) => {{
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                    nativeInputValueSetter.call(input, '{val_a3}');
                    input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                    input.dispatchEvent(new Event('change', {{ bubbles: true }}));
                }}
            """)
            mapping_table.locator('input[placeholder="如: order_id"]').nth(2).evaluate(f"""
                (input) => {{
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                    nativeInputValueSetter.call(input, '{val_f3}');
                    input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                    input.dispatchEvent(new Event('change', {{ bubbles: true }}));
                }}
            """)
            ...
            # 检查是否在报表列表中
            table_text = self.page.locator('.el-table').inner_text()
            if 'TEST_E2E' in table_text:
                self.record_result("报表配置", "PASS", f"报表配置创建成功")
                self.log(f"✅ 报表配置创建成功")
            else:
