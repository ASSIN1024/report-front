...
class E2ETestRunner:
    ...
    def test_report_config(self):
        ...
        try:
            ...
            # 填写报表基本信息
            self.page.fill('input[placeholder="如: SALES_REPORT"]', self.report_code)
            self.log(f"✅ 已填写报表编码: {self.report_code}")

            self.page.fill('input[placeholder="如: 销售报表"]', self.report_config_name)
            self.log("✅ 已填写报表名称")

            # 选择FTP配置
            self.page.locator('label:has-text("关联FTP") + .el-form-item__content .el-select').click()
            self.page.wait_for_timeout(500)
            self.page.locator('.el-select-dropdown__item:visible').first.click()
            self.log("✅ 已选择FTP配置")

            # 填写文件匹配
            self.page.fill('input[placeholder="如: sales_*.xlsx"]', 'test_flow_e2e*.xlsx')
            self.log("✅ 已填写文件匹配")

            # 填写输出表名
            self.page.locator('input[placeholder="如: t_sales_data"]').fill(self.output_table)
            self.log(f"✅ 已填写输出表名: {self.output_table}")
            ...
            for i in range(3):
                ...
                self.page.wait_for_timeout(500)
            self.log("✅ 已点击添加映射按钮3次")

            # 定位列映射表格
            mapping_table = self.page.locator('.el-card').nth(2).locator('.el-table')

            # 第一行
            mapping_table.locator('input[placeholder="如: A"]').nth(0).click()
            mapping_table.locator('input[placeholder="如: A"]').nth(0).fill('A')
            self.page.wait_for_timeout(100)
            mapping_table.locator('input[placeholder="如: order_id"]').nth(0).click()
            mapping_table.locator('input[placeholder="如: order_id"]').nth(0).fill('col_a')
            self.page.wait_for_timeout(100)
            mapping_table.locator('.el-select').nth(0).click()
            self.page.wait_for_timeout(300)
            self.page.locator('.el-select-dropdown__item:visible').filter(has_text="字符串").first.click()
            self.page.wait_for_timeout(100)
            self.log("✅ 已填写第一行映射")

            # 第二行
            mapping_table.locator('input[placeholder="如: A"]').nth(1).click()
            mapping_table.locator('input[placeholder="如: A"]').nth(1).fill('B')
            self.page.wait_for_timeout(100)
            mapping_table.locator('input[placeholder="如: order_id"]').nth(1).click()
            mapping_table.locator('input[placeholder="如: order_id"]').nth(1).fill('col_b')
            self.page.wait_for_timeout(100)
            mapping_table.locator('.el-select').nth(1).click()
            self.page.wait_for_timeout(300)
            self.page.locator('.el-select-dropdown__item:visible').filter(has_text="整数").first.click()
            self.page.wait_for_timeout(100)
            self.log("✅ 已填写第二行映射")

            # 第三行
            mapping_table.locator('input[placeholder="如: A"]').nth(2).click()
            mapping_table.locator('input[placeholder="如: A"]').nth(2).fill('C')
            self.page.wait_for_timeout(100)
            mapping_table.locator('input[placeholder="如: order_id"]').nth(2).click()
            mapping_table.locator('input[placeholder="如: order_id"]').nth(2).fill('col_c')
            self.page.wait_for_timeout(100)
            mapping_table.locator('.el-select').nth(2).click()
            self.page.wait_for_timeout(300)
            self.page.locator('.el-select-dropdown__item:visible').filter(has_text="小数").first.click()
            self.page.wait_for_timeout(100)
            self.log("✅ 已填写第三行映射")

            self.save_screenshot("12_report_mappings")

            # 点击保存按钮 - 使用 page-header 下的 primary 类型按钮
            page_header = self.page.locator('.page-header')
            save_btn = page_header.locator('button.el-button--primary')
            self.log(f"保存按钮可见: {save_btn.is_visible()}")
            save_btn.click()
            self.log("✅ 已点击保存按钮")
            ...
            # 检查是否有错误提示
            error_visible = self.page.is_visible('.el-form-item__error')
            if error_visible:
                error_text = self.page.locator('.el-form-item__error').first.inner_text()
                self.log(f"⚠️ 表单错误: {error_text}")
                self.save_screenshot("13a_form_error")

            # 检查是否有成功消息
            success_visible = self.page.is_visible('.el-message--success')
            if success_visible:
                success_text = self.page.locator('.el-message--success').first.inner_text()
                self.log(f"✅ 成功消息: {success_text}")
            ...
            if self.report_code in table_text:
                ...
                self.log("❌ 报表配置创建失败")
                self.log(f"当前表格内容: {table_text[:200]}")
        ...
        try:
            ...
            result = self.page.evaluate("""
                () => {
                    ...
                    const scanBtn = buttons.find(b => b.textContent.includes('立即扫描'));
                    if (scanBtn) {
                        scanBtn.click();
                        return true;
                    }
            ...
            if result:
                ...
                if self.page.is_visible('.el-message-box'):
    ...
    def test_data_validation(self):
        ...
        try:
            ...
            options = self.page.locator('.el-select-dropdown__item:visible').all()
            table_found = False
