...
class E2ETestRunner:
    ...
    def test_report_config(self):
        ...
        try:
            ...
            # 选择FTP配置
            self.page.locator('.el-form:has(input[placeholder="如: SALES_REPORT"]) .el-select').click()
            self.page.wait_for_timeout(500)
            ...
            # 获取列映射表格中的输入框
            # Excel列输入框
            excel_col_inputs = self.page.locator('.el-table').nth(1).locator('input[placeholder="如: A"]')
            # 字段名称输入框
            field_name_inputs = self.page.locator('.el-table').nth(1).locator('input[placeholder="如: order_id"]')

            # 填写第一行
            excel_col_inputs.nth(0).click()
            excel_col_inputs.nth(0).type('A')
            field_name_inputs.nth(0).click()
            field_name_inputs.nth(0).type('col_a')
            self.log("✅ 已填写第一行映射")

            # 选择第一行的字段类型
            self.page.locator('.el-table').nth(1).locator('.el-select').nth(0).click()
            self.page.wait_for_timeout(300)
            self.page.locator('.el-select-dropdown__item:has-text("字符串")').click()
            self.log("✅ 已选择第一行字段类型")

            # 填写第二行
            excel_col_inputs.nth(1).click()
            excel_col_inputs.nth(1).type('B')
            field_name_inputs.nth(1).click()
            field_name_inputs.nth(1).type('col_b')
            self.log("✅ 已填写第二行映射")

            # 选择第二行的字段类型
            self.page.locator('.el-table').nth(1).locator('.el-select').nth(1).click()
            self.page.wait_for_timeout(300)
            self.page.locator('.el-select-dropdown__item:has-text("整数")').click()
            self.log("✅ 已选择第二行字段类型")

            # 填写第三行
            excel_col_inputs.nth(2).click()
            excel_col_inputs.nth(2).type('C')
            field_name_inputs.nth(2).click()
            field_name_inputs.nth(2).type('col_c')
            self.log("✅ 已填写第三行映射")

            # 选择第三行的字段类型
            self.page.locator('.el-table').nth(1).locator('.el-select').nth(2).click()
            self.page.wait_for_timeout(300)
