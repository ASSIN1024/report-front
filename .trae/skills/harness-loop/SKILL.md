---
name: harness-loop
description: "长时间任务循环，使用 /loop (Claude Code dynamic mode) 和 ScheduleWakeup 每次唤醒时重新进入新鲜上下文。内部通过Agent调用harness-work。触发词：long-running、loop、wake-up、autonomous。不要用于：一次性任务执行、审查、发布、规划。"
...
description-zh: "长时间任务循环，使用 /loop 和 ScheduleWakeup 每次唤醒时重新进入新鲜上下文。内部通过Agent调用harness-work。触发词：long-running、loop、wake-up、autonomous。不要用于：一次性任务执行、审查、发布、规划。"
