...
description: "使用 Keep a Changelog + GitHub 的通用发布自动化。单次确认后端到端自动化：版本检测、CHANGELOG升级、标签、GitHub Release。触发词：release、version bump、publish。不要用于：实现、审查、规划、设置。"
...
description-zh: "使用 Keep a Changelog + GitHub 的通用发布自动化。单次确认后端到端自动化：版本检测、CHANGELOG升级、标签、GitHub Release。触发词：release、version bump、publish。不要用于：实现、审查、规划、设置。"
